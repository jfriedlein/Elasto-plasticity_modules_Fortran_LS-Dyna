...
c
     ...
     &                            n_n1, gamma_k, hardening_type, cm )
c
      use Tensor
      use cm_manager
...
c
    ...
    !&                 - (HillT_H*stress).dya.(stress*HillT_H) )
     ...
     &				 + 2. * shearMod_mu *deviatoric_I4(Eye)
		!E_e = invert( invert(d_Tt_d_eps) + gamma_k * N_four );
  !
		!get_tangent_C = E_e
  !   &                    - 1. / ( n_n1 * E_e * n_n1 - sqrt( 2./3. )
  !   &                      * get_d_R_d_gamma( gamma_k, alpha_k, 
  !   &                                       hardening_type, cm, hsv ) )
  !   &                      * ( (E_e*n_n1).dya.(n_n1*E_e) )
          
      get_tangent_C = d_Tt_d_eps
      write(*,*) "Just testing, missing invert(fourth order)"
c      
